...
{
    internal static class LogicNetworkWebEditor
    {
        ...
        private static readonly Dictionary<int, WeakReference<LogicNetworkEmitter>> registered = new Dictionary<int, WeakReference<LogicNetworkEmitter>>();
        private static readonly LogicNetworkEditorSessionRegistry<LogicNetworkEmitter, object, WebEditorSaveRequest> sessions =
            new LogicNetworkEditorSessionRegistry<LogicNetworkEmitter, object, WebEditorSaveRequest>();
        ...
        public static void Open(LogicNetworkEmitter logic)
        {
            if (logic == null)
            {
                return;
            ...
            if (activePort <= 0)
            {
                Debug.LogWarning("LogicNetwork web editor could not start local server.");
                return;
            }
            ...
            lock (sync)
            {
                registered[id] = new WeakReference<LogicNetworkEmitter>(logic);
        ...
        public static void RefreshCachedStateIfActive(LogicNetworkEmitter logic)
        {
            if (logic == null)
            {
                return;
        ...
        public static void RefreshRuntimeSignalsIfActive(LogicNetworkEmitter logic)
        {
            if (logic == null)
            {
                return;
        ...
        public static void ApplyPending(LogicNetworkEmitter logic)
        {
            if (logic == null)
            {
                return;
        ...
        private static void EnsureServer()
        {
            ...
            for (int port = FirstPort; port <= LastPort; port++)
            {
                try
                {
                    ...
                    {
                        IsBackground = true,
                        Name = "LogicNetworkWebEditor"
                    };
                    listenerThread.Start();
                    Debug.Log($"LogicNetwork web editor listening on http://127.0.0.1:{port}/");
                    return;
                }
                catch (Exception ex)
                {
                    Debug.LogWarning($"LogicNetwork web editor skipped port {port}: {ex.Message}");
                }
            }
        }
        ...
        private static void ListenLoop()
        {
            while (listener != null && listener.IsListening)
            {
                ...
                {
                    Debug.LogWarning($"LogicNetwork web editor request failed: {ex.Message}");
                }
            }
        }
        ...
        private static WebEditorState BuildLightState(LogicNetworkEmitter logic)
        {
            if (logic == null)
            {
                return new WebEditorState();
        ...
        private static WebEditorState BuildFullState(LogicNetworkEmitter logic)
        {
            if (logic == null)
            {
                return new WebEditorState();
        ...
        private static LogicNetworkEmitter ResolveLogic(int id)
        {
            lock (sync)
            {
                if (registered.TryGetValue(id, out WeakReference<LogicNetworkEmitter> reference) &&
                    reference.TryGetTarget(out LogicNetworkEmitter logic) &&
                    logic != null)
                {
                    return logic;
                }
        ...
        private static string GetEditorPath()
        {
            ...
            return Path.Combine(directory ?? string.Empty, "WebEditor", "logicnetwork-editor.html");
        }
        ...
        private static bool TryOpenTopmostBrowserWindow(string url, int id, string windowTitle)
        {
            ...
            {
                Debug.LogWarning($"LogicNetwork web editor could not open topmost browser window: {ex.Message}");
                return false;
            }
        }
        ...
        private static void SetEditorWindowTopmost(System.Diagnostics.Process process, int id, string windowTitle)
        {
            ...
            {
                Debug.LogWarning($"LogicNetwork web editor could not set browser window topmost: {ex.Message}");
            }
        }
        ...
        private static IntPtr FindEditorWindow(int id, string windowTitle)
        {
            ...
            EnumWindows((handle, _) =>
            {
                ...
                bool titleLooksLikeEditor = title.IndexOf(global::LogicNetwork.LogicNetworkStrings.EditorTitle, StringComparison.OrdinalIgnoreCase) >= 0 ||
        ...
        private static string GetExpectedWindowTitle(WebEditorState state)
        {
            ...
            return string.IsNullOrEmpty(buildingName)
                ? global::LogicNetwork.LogicNetworkStrings.EditorTitle
                : buildingName + " - " + global::LogicNetwork.LogicNetworkStrings.EditorTitle;
        }
