...
{
    internal sealed class StorageNetworkInputFieldMarker : MonoBehaviour
    {
    }

    internal static class StorageNetworkInputBuilder
    {
            ...
            Vector2 textAreaPadding)
        {
            ...
            TMP_InputField input = CreateInputCore<TMP_InputField>(root, value, fontSize, alignment, backgroundColor, textColor, textAreaPadding);
            Finish(root, input);
            return input;
        }

        public static KInputTextField CreateKNumberInput(
            Transform parent,
            string name,
            string value,
            float width,
            ...
            Color textColor,
            Vector2 textAreaPadding,
            bool addFocusOutline)
        {
            return CreateKNumberInput(
                parent,
                name,
                value,
                width,
        ...
        public static KInputTextField CreateKNumberInput(
            ...
            bool addFocusOutline)
        {
            GameObject root = CreateRoot(name, parent, borderColor, borderSprite, width, height);
            root.SetActive(false);
            if (addFocusOutline)
            {
                Outline focusOutline = root.AddComponent<Outline>();
                focusOutline.effectColor = new Color(0.36f, 0.54f, 0.72f, 1f);
                focusOutline.effectDistance = new Vector2(1.5f, -1.5f);
                focusOutline.useGraphicAlpha = false;
                focusOutline.enabled = false;
            }

            KInputTextField input = CreateInputCore<KInputTextField>(root, value, fontSize, alignment, backgroundColor, textColor, textAreaPadding);
        ...
        private static T CreateInputCore<T>(
            GameObject root,
            string value,
            int fontSize,
            TextAlignmentOptions alignment,
            Color backgroundColor,
            Color textColor,
            Vector2 textAreaPadding) where T : TMP_InputField
        {
            ...
            root.AddComponent<StorageNetworkInputFieldMarker>();
            return input;
        }
        ...
        private static void Finish(GameObject root, TMP_InputField input)
        {
            root.SetActive(true);
